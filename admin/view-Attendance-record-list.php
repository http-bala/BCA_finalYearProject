...
function outime($id,$customdate,$con){

  $getoutime = "SELECT * FROM `attend-record` WHERE `emp_id`='$id' AND `attend_date`= '$customdate' AND `typeofpunching` = 'out-time'; ";
  $query = mysqli_query($con,$getoutime) or die("not working");
  $output = mysqli_fetch_assoc($query);
  // $out_time_data = "pending";
  if ($output) {
    // $out_time_data = $output['attend_time'];
    $out_time_data = strtotime($output['attend_time']);
      return date("h:i:sa",$out_time_data);
  }else{
     $out_time_data = "pending";
    return $out_time_data;
  }
    
}

function workhours($intime,$outitme){

  if($outitme == "pending"){
      $val ="not enter the out time";
      return $val;
  }else {
    $startTime = strtotime($intime);
    $endtime = strtotime($outitme);
...
function latetime($intime,$empuser,$con){
  ...
  $startTime = strtotime($intime);
  $shittime = strtotime($correct_time);
  
  $diff =  $shittime - $startTime ;
  
  $hours = floor($diff / 3600);
  $Minutes = floor(($diff % 3600) / 60);
  $Seconds = $diff % 60;

  $timevalue = sprintf("%02d:%02d:%02d",abs($hours),abs($Minutes),abs($Seconds));
      ...
      <div class="layout-page">
        ...
        <div class="content-wrapper">
          ...
          <div class="container-xxl flex-grow-1 container-p-y">
            <h4 class="py-3 mb-4"><span class="text-muted fw-light">ATTEND/</span>IN-TIME </h4>
            ...
            <!-- ===================================================== -->
              ...
              <tbody>
                ...
                if (isset($_POST['form']) && isset($_POST['to'])) {
                  $form = $_POST['form'];
                  $to = $_POST['to'];
                  $sql = "SELECT * FROM `attend-record` WHERE `dt_record` BETWEEN '$form' AND '$to'";
                }else {
                  $sql = "SELECT * FROM `attend-record` WHERE `typeofpunching` = 'in-time'";
                }
                ...
                if (mysqli_num_rows($result) > 0) {
                  ...
                  while ($data = mysqli_fetch_assoc($result)) {
                    $attend = strtotime($data['attend_time']);
                ?><tr>
                <td><?php  echo $a += 1; ?></td>

                      <td><?php echo $data['emp_id'] ?></td>
                      <td><?php echo $data['emp_user'] ?></td>
                      <td><?php echo $data['attend_date']?></td>
                      <td class="text-success fw-bold "><?php echo date("h:i:sa",$attend) ?></td>
                      <td class="text-success fw-bold"> <?php echo $newoutput = outime( $data['emp_id'],$data['attend_date'],$con); ?></td>
                      <td class="text-warning fw-bold"><?php  echo $work = workhours($data['attend_time'],$newoutput) ?></td>
                      <td class="text-danger fw-bold"><?php echo $work =latetime($data['attend_time'],$data['emp_user'] ,$con); ?></td>
                    </tr>
                <?php
                  }
                }
